...
//            e.printStackTrace();
     ...
     * @throws IOException
     */
    public Path getExecutionTrace(String classUnderTest) throws IOException {
        try {
...
//                                curTarget = Population.currentTargetPath.;
//                            else
...
//            }
            // add coveredBranches
            List<BranchTarget> coveredBranches;
            coveredBranches = new LinkedList<BranchTarget>();
            Iterator j = trace.iterator();
            while (j.hasNext()) {
                BranchTarget branch = new BranchTarget(((Integer) j.next()).intValue());
                coveredBranches.add(branch);
            }
            Path coveredPath = new Path(coveredBranches);
            return coveredPath;
